...
export default function Home() {
  ...
  const [isNewProjectLibraryModalOpen, setIsNewProjectLibraryModalOpen] =
    useState(false);
  ...
  return (
    <div className="min-h-screen flex flex-col">
          ...
          <div className="bg-white rounded-lg w-full max-w-2xl mx-4">
            ...
            <div className="p-6">
              <p className="text-gray-600 mb-8">
                Choose one of the options below to continue. You will be prompted to enter name and other details in
                further steps.
              </p>
      ...
      {isNewProjectLibraryModalOpen && (
        <div className="fixed inset-0 bg-gray-300 bg-opacity-95 flex items-start justify-center pt-16 z-50">
          <div className="bg-white rounded-lg w-full max-w-2xl mx-4">
            <div className="flex items-center bg-blue-400 justify-between p-6 border-b">
              <h2 className="text-2xl font-semibold text-gray-800">Create project: Choose a source</h2>
              <button onClick={() => setIsNewProjectLibraryModalOpen(false)} className="text-gray-500 hover:text-gray-700">
                <X className="w-6 h-6" />
              </button>
            </div>
      ...
      {/* Upload XLSForm Popup */}
      {isUploadPopupOpen && (
        <UploadXLSFormPopup onClose={() => setIsUploadPopupOpen(false)} />
      )}

      {/* Import XLSForm Popup */}
      {isCreateProjectPopupOpen && (
        <CreateProjectPopup onClose={() => setIsCreateProjectPopupOpen(false)} />
      )}

      {/* Template Popup */}
      {isTemplatePopupOpen && (
        <ImportXLSForm onClose={() => setIsTemplatePopupOpen(false)} />
      )}
    </div>
  )
}
